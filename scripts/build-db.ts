#!/usr/bin/env tsx
/**
 * Database builder for Cambodia Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Cambodia Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Cambodian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'KH');
    insertMeta.run('source', 'cdc.gov.kh + camlawbox.com');
