#!/usr/bin/env tsx
/**
 * Cambodian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://camlawbox.com';
const PORTAL_NAME = 'Cambodia Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/cambodian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Cambodian Law MCP — Data Freshness Check');
