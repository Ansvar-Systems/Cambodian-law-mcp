#!/usr/bin/env tsx
/**
 * Drift detection for Cambodia Law MCP.
 *
 * Checks if upstream Cambodia Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Cambodia Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'cambodian-law-mcp/1.0 drift-detect' },
      });
