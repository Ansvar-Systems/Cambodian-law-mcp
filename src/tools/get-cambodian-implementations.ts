/**
 * get_cambodian_implementations — Find Cambodian statutes implementing a specific EU directive/regulation
...
export interface GetCambodianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface CambodianImplementationResult {
  document_id: string;
...
export async function getCambodianImplementations(
  db: InstanceType<typeof Database>,
  input: GetCambodianImplementationsInput,
): Promise<ToolResponse<CambodianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as CambodianImplementationResult[];
