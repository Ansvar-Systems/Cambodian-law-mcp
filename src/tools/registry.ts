/**
 * Tool registry for Cambodia Law MCP Server.
...
import { getCambodianImplementations, type GetCambodianImplementationsInput } from './get-cambodian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including Cambodia Law (National Council for Law Reporting). ' +
    ...
    'Call this FIRST when you need to understand what Cambodian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Cambodian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section/article) from a Cambodian statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate a Cambodian legal citation against the database — zero-hallucination check. ' +
      ...
      'Supports formats: "Section 25, Data Protection Act 2019", "s 25, DPA 2019", "Article 31, Constitution of Cambodia 2010".',
    inputSchema: {
      type: 'object',
      properties: {
        citation: {
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Cambodian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Cambodia?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Cambodian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Cambodian statute or provision is currently in force, amended, repealed, partially suspended, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU/international legal basis that a Cambodian statute aligns with or references. ' +
      'Cambodia is not an EU member but several Cambodian laws were influenced by EU regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Cambodian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_cambodian_implementations',
    description:
      'Find all Cambodian statutes that align with or implement a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Cambodian statutes. ' +
      'Note: Cambodia implements international standards through domestic legislation influenced by international frameworks.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that have Cambodian implementing/aligning legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_cambodian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents with Cambodian aligning legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU/international legal basis for a SPECIFIC provision within a Cambodian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU/international alignment status for a Cambodian statute or provision. ' +
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_cambodian_implementations':
          result = await getCambodianImplementations(db, args as unknown as GetCambodianImplementationsInput);
          break;
