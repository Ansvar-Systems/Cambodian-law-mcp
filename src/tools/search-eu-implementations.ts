/**
 * search_eu_implementations — Search EU directives/regulations that have Cambodian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_cambodian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  cambodian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as cambodian_statute_count
  ...
  if (input.has_cambodian_implementation) {
    sql += ' HAVING cambodian_statute_count > 0';
  }
